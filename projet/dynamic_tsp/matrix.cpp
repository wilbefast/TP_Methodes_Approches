...
int matrix_unit()
{
  ...
  for(row = 0; row < m.n_rows; row++)
    for(col = 0; col < m.n_cols; col++)
      m.t[row][col] = (int)(col + m.n_cols*row);

  // read
  for(row = 0; row < m.n_rows; row++)
    for(col = 0; col < m.n_cols; col++)
      ASSERT(m.t[row][col] == (int)(col + m.n_cols*row),
...
void calloc_matrix(matrix_t* m)
{
  ...
  m->t = (int**)calloc(m->n_rows, sizeof (int*));
  m->t[0] = (int*)calloc(m->n_rows * m->n_cols, sizeof (int));
